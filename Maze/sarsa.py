...
def sarsa(env, num_episodes=50, alpha=0.1, gamma=0.99, epsilon=0.1, render_interval=10, save_video=False, video_path="sarsa_train.mp4"):
...
q_table, rewards = sarsa(env, num_episodes, alpha, gamma, epsilon, render_interval, save_video=True)
...
def play(env, q_table):
    ...
    if len(frames)>0:
        imageio.mimsave("sarsa_play.mp4", frames, fps=5)
